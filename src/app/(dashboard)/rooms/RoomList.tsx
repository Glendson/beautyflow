            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {rooms.map((r) => (
                <tr key={r.id} className="hover:bg-gray-50 transition-colors group">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 capitalize">{r.type === 'room' ? '🚪 Private Room' : '🪑 Open Station'}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                    <button onClick={() => handleDelete(r.id)} disabled={loadingId === r.id} className="text-red-600 hover:text-red-900 opacity-0 group-hover:opacity-100 transition-opacity disabled:opacity-50">
