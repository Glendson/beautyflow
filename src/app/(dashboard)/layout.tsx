      ...
      <aside className="w-68 flex flex-col bg-white border-r border-gray-200/60" style={{ boxShadow: "1px 0 3px rgba(0,0,0,0.03)" }}>
        ...
        <div className="h-16 flex items-center px-6 border-b border-gray-100/80">
          <div className="flex items-center gap-2.5">
            ...
            <span className="text-lg font-bold tracking-tight" style={{ background: "linear-gradient(135deg, #ec4899, #8b5cf6)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>
              BeautyFlow
            </span>
          </div>
        </div>
        ...
        <nav className="flex-1 px-3 py-4 space-y-0.5 overflow-y-auto">
          <p className="px-3 py-2 text-[0.6875rem] font-semibold uppercase tracking-wider text-gray-400">Menu</p>
          {NAV_ITEMS.map((item) => {
            ...
            return (
              ...
              >
                <span className={`transition-colors duration-150 ${isActive ? "text-pink-600" : "text-gray-400 group-hover:text-gray-600"}`}>
                  {item.icon}
                </span>
                {item.label}
                {isActive && (
                  <span className="ml-auto w-1.5 h-1.5 rounded-full" style={{ background: "linear-gradient(135deg, #ec4899, #8b5cf6)" }} />
                )}
              </Link>
            );
          })}
        ...
        <div className="p-3 border-t border-gray-100/80">
          <form action={logoutAction}>
            <button
              type="submit"
              className="w-full flex items-center gap-3 px-3 py-2.5 text-sm font-medium text-gray-500 rounded-lg hover:bg-red-50 hover:text-red-600 transition-all duration-150"
            >
              <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor">
